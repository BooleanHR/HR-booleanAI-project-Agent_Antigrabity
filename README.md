...
| **Level 3**<br>(이벤트 기반) | **Hooks (훅)** | 특정 이벤트(예: 파일 저장, 커밋 생성, 에이전트 시작/종료)가 발생할 때 **자동으로 트리거되어 실행되는 스크립트나 자동화 작업**입니다. | `hooks.json`이나 특정 설정 파일에 이벤트와 실행할 동작을 매핑합니다. 사용자의 명시적 프롬프트 없이도 **정해진 라이프사이클에 맞춰 백그라운드에서 자동 실행**됩니다. |
